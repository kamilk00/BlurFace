...
def faceDetection(faceNet, frame):
...
face_pbtxt = dirPath + "\FaceBlur\opencv_face_detector.pbtxt"
face_pb = dirPath + "\FaceBlur\opencv_face_detector_uint8.pb"
faceNet = cv2.dnn.readNet(face_pb, face_pbtxt)

while cv2.waitKey(1) < 0:

    _, img = video.read()
    faceBoxes = faceDetection(faceNet, img)
